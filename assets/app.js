(function () {
  ...
  var AUTH_KEY = "personal-calendar-board-auth";
  var SUPABASE_URL = "https://tkevwifbkxesblpxqydt.supabase.co";
  var SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRrZXZ3aWZia3hlc2JscHhxeWR0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODMwNzU5NTAsImV4cCI6MjA5ODY1MTk1MH0.0IxmcxU0T-xyL03aNHxydExqVykqji9JfIDuZM5k0Ew";
  var defaultTasks = [
    {
      id: "t1",
  ...
  function toCloudTask(task) {
    return {
      title: task.title,
      module: task.module,
      time_text: task.time || "今天",
      priority: task.priority || "中",
      reminder: task.reminder || "待设置",
      done: !!task.done,
  ...
  function fromCloudTask(row) {
    return {
      ...
      title: row.title,
      module: row.module,
      time: row.time_text || "今天",
      priority: row.priority || "中",
      reminder: row.reminder || "待设置",
      done: !!row.done,
      overdue: !!row.overdue,
      source: row.source || "cloud",
      tagClass: inferTagClass(row.module, !!row.overdue)
    };
  }

  function inferTagClass(module, overdue) {
    if (overdue) return "overdue";
    if (module === "投资提醒") return "warn";
    if (module === "健康记录" || module === "日常运动") return "health";
    if (module === "日常提醒") return "life";
    return "";
  }

  function loadAuth() {
    try {
      var saved = window.localStorage.getItem(AUTH_KEY);
  ...
  function isLoggedIn() {
    return !!(authState && authState.access_token && authState.user && authState.user.id);
  }
  ...
  function renderAuthState() {
    if (!authStatus || !syncBadge) return;
    if (isLoggedIn()) {
      syncBadge.textContent = "云端同步";
      authStatus.innerHTML = "已登录：<strong>" + (authState.user.email || "当前账号") + "</strong>。新增任务和打勾会同步到 Supabase。";
      if (authEmail) authEmail.value = authState.user.email || "";
    } else {
      syncBadge.textContent = "本地模式";
      authStatus.textContent = "未登录：当前任务只保存在本设备。登录后，电脑和手机会同步同一账号的任务。";
    }
  }
  ...
  async function signUp(email, password) {
    var data = await requestJson(SUPABASE_URL + "/auth/v1/signup", {
      method: "POST",
      headers: authHeaders(),
    ...
    if (data.access_token) {
      ...
      showToast("注册并登录成功，已开启云同步。");
    } else {
      showToast("注册成功，请先去邮箱确认邮件，再回来登录。");
    }
  }

  async function signIn(email, password) {
    var data = await requestJson(SUPABASE_URL + "/auth/v1/token?grant_type=password", {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ email: email, password: password })
    });
    saveAuth(data);
    await syncAfterLogin();
    showToast("登录成功，已从云端同步任务。");
  }

  async function signOut() {
    saveAuth(null);
    showToast("已退出登录，当前设备仍保留本地任务。");
  }

  async function fetchCloudTasks() {
    var rows = await requestJson(SUPABASE_URL + "/rest/v1/tasks?select=*&order=created_at.asc", {
      method: "GET",
      headers: authHeaders()
    });
    return rows.map(fromCloudTask);
  }

  async function insertCloudTask(task) {
    if (!isLoggedIn()) return null;
    var payload = Object.assign(toCloudTask(task), { user_id: authState.user.id });
    var rows = await requestJson(SUPABASE_URL + "/rest/v1/tasks", {
      method: "POST",
      headers: authHeaders({ "Prefer": "return=representation" }),
      body: JSON.stringify(payload)
    });
    return rows && rows[0] ? fromCloudTask(rows[0]) : task;
  }

  async function updateCloudTask(task) {
    if (!isLoggedIn() || !task.id || String(task.id).indexOf("ai-") === 0 || String(task.id).indexOf("t") === 0) return;
    await requestJson(SUPABASE_URL + "/rest/v1/tasks?id=eq." + encodeURIComponent(task.id), {
      method: "PATCH",
      headers: authHeaders(),
      body: JSON.stringify({
        done: !!task.done,
  ...
  async function seedCloudTasks(localTasks) {
    if (!isLoggedIn()) return [];
    var payload = localTasks.map(function (task) {
      return Object.assign(toCloudTask(task), { user_id: authState.user.id });
    });
  ...
  async function syncAfterLogin() {
    var localSnapshot = tasks.slice();
    var cloudTasks = await fetchCloudTasks();
    if (!cloudTasks.length && localSnapshot.length) {
      cloudTasks = await seedCloudTasks(localSnapshot);
    }
    tasks = normalizeOverdueTasks(cloudTasks.length ? cloudTasks : localSnapshot);
    saveTasks();
    renderTasks();
    updateStats();
    renderDetail(activeLibraryId);
  }

  async function addTaskFromStructured(structured) {
    var newTask = {
      id: "ai-" + Date.now(),
    ...
    if (isLoggedIn()) {
      try {
        newTask = await insertCloudTask(newTask);
      } catch (error) {
        showToast("云端保存失败，已先保存在本地：" + error.message);
      }
    }
    tasks.push(newTask);
    saveTasks();
  ...
  document.addEventListener("click", function (event) {
    ...
    if (taskItem && event.target.closest(".task-check")) {
      ...
      if (task) {
        ...
        updateCloudTask(task).catch(function (error) {
          showToast("云端同步失败，本地已保留：" + error.message);
        });
      }
      return;
    }
  ...
  if (chatForm) {
    chatForm.addEventListener("submit", async function (event) {
      ...
      await addTaskFromStructured(structured);
  ...
  document.getElementById("quickAdd").addEventListener("click", function () {
    ...
    Promise.resolve(isLoggedIn() ? insertCloudTask(quickTask) : quickTask)
      .then(function (savedTask) {
        tasks.push(savedTask || quickTask);
        saveTasks();
        renderTasks();
        updateStats();
        switchScreen("tasks");
        showToast(isLoggedIn() ? "已新增并同步到云端。" : "已新增到本地任务库。");
      })
  ...
  updateStats();
  if (isLoggedIn()) {
    syncAfterLogin().catch(function (error) {
